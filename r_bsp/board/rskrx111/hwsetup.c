...
static void output_ports_configure(void)
{
    ...
    /* RSPI signals, chip selects, peripheral reset */
    PORTA.PODR.BIT.B4  = 1;     /* Using GPIO as slave select, set high (not selected). */
    PORTA.PDR.BIT.B4   = 1;     /* Set as output. */

    PORTA.PODR.BYTE &= (uint8_t)0xB7;    /* Clear data for RSPI port bits */
    PORTB.PODR.BYTE &= (uint8_t)0xFE;
    PORTA.PMR.BYTE  &= (uint8_t)0xB7;    /* RSPI port bits GPIO for now */
    PORTB.PMR.BYTE  &= (uint8_t)0xFE;
